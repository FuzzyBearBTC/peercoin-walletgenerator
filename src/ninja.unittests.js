	...
	var ut = ninja.unitTests = {
		...
		synchronousTests: {
			...
			testGetPeercoinPrivateKeyByteArray: function () {
				...
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinPrivateKeyByteArray().toString() != bytes.toString()) {
					return false;
				}
				return true;
			},
			...
			testBase64ToECKey: function () {
				...
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinBase64Format() != "KSZlw4ckGK3x2n/6OmRvLwYCJG2mCYqR0inDIVDycYs=") {
					return false;
				}
				return true;
			},
			testHexToECKey: function () {
				...
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinHexFormat() != "292665C3872418ADF1DA7FFA3A646F2F0602246DA6098A91D229C32150F2718B") {
					return false;
				}
				return true;
			},
			testCompressedWifToECKey: function () {
				var key = "KxbhchnQquYQ2dfSxz7rrEaQTCukF4uCV57TkamyTbLzjFWcdi3S";
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinWalletImportFormat() != "KxbhchnQquYQ2dfSxz7rrEaQTCukF4uCV57TkamyTbLzjFWcdi3S"
						|| btcKey.getPubPoint().compressed != true) {
					return false;
				}
				return true;
			...
			testWifToECKey: function () {
				var key = "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb";
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinWalletImportFormat() != "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb") {
					return false;
				}
				return true;
			},
			testBrainToECKey: function () {
				...
				var btcKey = new Peercoin.ECKey(bytes);
				if (btcKey.getPeercoinWalletImportFormat() != "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb") {
					return false;
				}
				return true;
			...
			testMini30CharsToECKey: function () {
				...
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinWalletImportFormat() != "5JrBLQseeZdYw4jWEAHmNxGMr5fxh9NJU3fUwnv4khfKcg2rJVh") {
					return false;
				}
				return true;
			},
			...
			testBugWithLeadingZeroBytePublicKey: function () {
				...
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinAddress() != "1M6dsMZUjFxjdwsyVk8nJytWcfr9tfUa9E") {
					return false;
				}
				return true;
			},
			testBugWithLeadingZeroBytePrivateKey: function () {
				...
				var btcKey = new Peercoin.ECKey(key);
				if (btcKey.getPeercoinAddress() != "1NAjZjF81YGfiJ3rTKc7jf1nmZ26KN7Gkn") {
					return false;
				}
				return true;
			}
				...
				var decryptTest = function (test, i, onComplete) {
					ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(test[0], test[1], function (privBytes) {
						if (privBytes.constructor == Error) {
							...
							var btcKey = new Peercoin.ECKey(privBytes);
							var wif = !test[2].substr(0, 1).match(/[LK]/) ? btcKey.setCompressed(false).getPeercoinWalletImportFormat() : btcKey.setCompressed(true).getPeercoinWalletImportFormat();
							if (wif != test[2]) {
						...
						ninja.privateKey.BIP38GenerateECAddressAsync(intermediatePoint, false, function (address, encryptedKey) {
							ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(encryptedKey, pass, function (privBytes) {
								if (privBytes.constructor == Error) {
									...
									var btcKey = new Peercoin.ECKey(privBytes);
									var btcAddress = btcKey.getPeercoinAddress();
									if (address !== btcKey.getPeercoinAddress()) {
